-- CORRECCIÓN V2: Entrenadores principales NO requieren aprobación
...
-- 4. ELIMINAR política existente ANTES de crear nueva
DROP POLICY IF EXISTS "Approved coaches can create their own club" ON public.clubs;
DROP POLICY IF EXISTS "Entrenadores principales can create their own club" ON public.clubs;
...
-- 5. También actualizar política de actualización de clubs
DROP POLICY IF EXISTS "Approved coaches can update their own club" ON public.clubs;
DROP POLICY IF EXISTS "Entrenadores principales can update their own club" ON public.clubs;

CREATE POLICY "Entrenadores principales can update their own club" ON public.clubs
FOR UPDATE USING (auth.uid() = user_id AND (
  SELECT role FROM public.profiles WHERE id = auth.uid()
) = 'entrenador_principal');

-- 6. Y política de eliminación de clubs
DROP POLICY IF EXISTS "Approved coaches can delete their own club" ON public.clubs;
DROP POLICY IF EXISTS "Entrenadores principales can delete their own club" ON public.clubs;

CREATE POLICY "Entrenadores principales can delete their own club" ON public.clubs
FOR DELETE USING (auth.uid() = user_id AND (
  SELECT role FROM public.profiles WHERE id = auth.uid()
) = 'entrenador_principal');

-- Mensaje de confirmación
SELECT 'Corrección V2 aplicada exitosamente: Entrenadores principales ya no requieren aprobación' as resultado;
